...
def _run_sync(action: str, api_key: str, kwargs: dict) -> str:
    ...
    try:
        ...
        if action == "send_email":
            r = client.post(f"{base_url}/smtp/email", json={"sender": {"email": kwargs.get("from_email", "")}, "to": [{"email": kwargs.get("to", "")}], "subject": kwargs.get("subject", ""), "htmlContent": kwargs.get("html", "")}, headers=headers)
            ...
            r = client.post(f"{base_url}/contacts/import", json={"contacts": kwargs.get("contacts", [])}, headers=headers)
            return str(r.json())[:2000]
        else:
            return f"Error: Unknown action '{action}'. Available: send_email, create_template, send_template, get_stats, import_contacts"
